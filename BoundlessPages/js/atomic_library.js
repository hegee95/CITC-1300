...
document.addEventListener('DOMContentLoaded', () => {
    console.log("Atomic Library script loaded"); // Log 1: Script start

    // Try finding the specific grid, fall back to general item grid if on library page
    let gridContainer = document.querySelector('.library-grid');
    if (!gridContainer) {
        const generalGrid = document.querySelector('.item-grid');
        // Ensure we are actually on the library page before using the general grid
        if (generalGrid && document.querySelector('.page-library')) {
             console.warn("Using general '.item-grid' selector for library page.");
             gridContainer = generalGrid;
        } else {
            // Only log error if we expect a grid on this page
            if (document.querySelector('.page-library')) {
                 console.error("Library grid container (.library-grid or .item-grid) not found on Library page!");
            } else {
                 console.log("Not on library page or grid not found, script inactive.");
            }
