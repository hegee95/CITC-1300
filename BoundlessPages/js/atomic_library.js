...
document.addEventListener('DOMContentLoaded', () => {
    console.log("Atomic Library script loaded");

    const libraryGrid = document.querySelector('.library-grid'); // Ensure class matches HTML

    if (!libraryGrid) {
        // Try the general item-grid class if specific one isn't found
        const grid = document.querySelector('.item-grid');
        if (grid && document.querySelector('.page-library')) { // Check if we are on the library page
             console.warn("Using general '.item-grid' for library page.");
             setupGridListener(grid);
        } else {
            console.error("Library grid container not found!");
        }
        return; // Stop if no grid found specifically or generally on this page
    } else {
         console.log("Library grid found.");
         setupGridListener(libraryGrid); // Setup listener if specific grid found
    }
    ...
    function setupGridListener(gridContainer) {
        ...
        function setupPagination(gridItemElement) {
            ...
            const contentArea = popup?.querySelector('.synopsis'); // Specific content class
        ...
        gridContainer.addEventListener('click', (event) => {
            console.log("Click detected inside library grid. Target:", event.target);

            // Check if a book spine image was clicked
            // Use generic class names from the new HTML structure
